...
public class CesiumRuntime : ModuleRules
{
    public CesiumRuntime(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        string debugLibPath = libPathBase + "-Debug";
        string releaseLibPath = libPathBase + "-Release";
        ...
        string libPath = releaseLibPath;
        if (preferDebug) {
            if (Directory.Exists(debugLibPath)) {
                libPath = debugLibPath;
            } else {
                Console.WriteLine("Using release build of cesium-native because a debug build is not available.");
            }
        }

        // Add every file in the cesium-native lib directory
        string[] libs = Directory.GetFiles(libPath);
