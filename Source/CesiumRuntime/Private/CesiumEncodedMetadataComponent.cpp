...
static FORCEINLINE UMaterialFunctionMaterialLayer*
LoadMaterialLayer(const FName& Path) {
  if (Path == NAME_None)
    return nullptr;
...
void UCesiumEncodedMetadataComponent::GenerateMaterial() {
  ...
  if (FPackageName::DoesPackageExist(PackageName)) {
    ...
    // TODO: checkout asset if using version system
    // if (!UEditorAssetLibrary::CheckoutLoadedAsset(UnrealMaterial)) {
    //  return;
    //}

    GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()
        ->CloseAllEditorsForAsset(UnrealMaterial);
  }
  ...
  if (!Overwriting) {
    InputMaterial = NewObject<UMaterialExpressionFunctionInput>(UnrealMaterial);
  ...
  // Save the asset
  UEditorAssetLibrary::SaveLoadedAsset(UnrealMaterial, true);

  // Open updated material in editor.
  FAssetData assetData(UnrealMaterial);
