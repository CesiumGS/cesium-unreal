...
void FCesiumMetadataPickingSpec::Define() {
  ...
  Describe("GetPropertyTableValuesFromHit", [this]() {
    ...
    //        static_cast<int64_t>(pStructuralMetadata->propertyTables.size() -
    //        1);
    ...
    //      TestEqual(
    //          "scalar value",
    //          UCesiumMetadataValueBlueprintLibrary::GetInteger(scalarValue,
    //          0), scalarValues[i]);
  ...
  Describe("GetMetadataValuesForFace (Deprecated)", [this]() {
    BeforeEach([this]() {
      // Two disconnected triangles.
      std::vector<glm::vec3> positions{
          glm::vec3(-1, 1, 0),
