...
static MaterialNodeClassification
...
static void ClearAutoGeneratedNodes(
  ...
  MaterialNodeClassification Classification = ClassifyNodes(Layer);
...
static void GenerateMaterialNodes(
  ...
  for (const UMaterialExpression* SrcExpression : SrcCollection.Expressions) {
    ...
    if (CustomNode && CustomNode->GetDescription() == "Voxel Raymarch") {
  ...
  for (UMaterialExpression* NewExpression : MaterialState.AutoGeneratedNodes) {
    const TArray<FExpressionInput*>& ExpressionInputs =
        NewExpression->GetInputs();
  ...
  // SetMaterialAttributes needs to manage an internal list of which attributes
  // were selected.
...
void UCesiumVoxelMetadataComponent::GenerateMaterial() {
  ...
  ClearAutoGeneratedNodes(this->TargetMaterialLayer, MaterialState);
