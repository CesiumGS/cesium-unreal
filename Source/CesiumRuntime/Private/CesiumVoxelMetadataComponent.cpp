...
static void ClearAutoGeneratedNodes(
  ...
  if (pEyeAdaptationInverse) {
    for (FExpressionInputIterator It(pEyeAdaptationInverse); It; ++It) {
      if (It.Expression && It.Expression != pRaymarchNode) {
...
static void RemapUserConnections(
  ...
  UMaterialExpression* pEyeAdaptationInverse =
      Classification.pEyeAdaptationInverseNode;
  if (pEyeAdaptationInverse) {
    int32 inputCount = pEyeAdaptationInverse->CountInputs();
    for (int32 i = 0; i < inputCount; i++) {
      FExpressionInput* pInput = pEyeAdaptationInverse->GetInput(i);
      assert(pInput);

      MaterialGenerationState::ParameterConnections* pConnections =
          connectionInputMap.Find(TEXT("EyeAdaptationInverse"));
      if (pConnections) {
        const auto* pConnectionInput = pConnections->Find(FString::FromInt(i));
        if (!pConnectionInput) {
          return;
        }
        ...
        if (std::holds_alternative<const FExpressionInput*>(
          ...
          pInput->Expression = pUserNode->Expression;
          pInput->OutputIndex = pUserNode->OutputIndex;
        }
      }
    }
...
void UCesiumVoxelMetadataComponent::GenerateMaterial() {
  ...
  if (!Overwriting &&
      this->TargetMaterial->GetExpressionCollection().Expressions.Num()) {
    // Attempted to create a new material, but found an existing one with
    // the same name. The material generation breaks at this stage, so create an
    // actual new material.
    this->TargetMaterial = CreateMaterial(PackageName, MaterialName + "_0");
  }
