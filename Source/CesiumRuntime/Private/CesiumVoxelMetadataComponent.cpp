...
void UCesiumVoxelMetadataComponent::OnFetchMetadata(
  ...
  const std::string classId(TCHAR_TO_UTF8(*this->Description.ID));
...
/*static*/ const FString UCesiumVoxelMetadataComponent::_shaderPreviewTemplate =
    ...
    "//////////////////////////////\n"
    "// Any properties or statistics you add from the voxel class are declared here.\n"
    "//////////////////////////////\n"
    "%s"
    "\n\n"
    "//////////////////////////////\n"
...
static void GenerateMaterialNodes(
  ...
  UMaterialFunctionMaterialLayer* pTargetLayer =
  ...
  for (const UMaterialExpression* pSrcExpression : SrcCollection.Expressions) {
    ...
    const auto* pInputMaterial =
        Cast<UMaterialExpressionFunctionInput>(pSrcExpression);
    const auto* pSetAttributes =
        Cast<UMaterialExpressionSetMaterialAttributes>(pSrcExpression);
    const auto* pOutputMaterial =
        Cast<UMaterialExpressionFunctionOutput>(pSrcExpression);
    if (pInputMaterial || pSetAttributes || pOutputMaterial) {
      continue;
    }
  ...
  int32 SetMaterialAttributesOffset =
      pBreakFloat4Node->MaterialExpressionEditorX;
  NodeX = DataSectionX;
  NodeY = DataSectionY;
  ...
  for (const FCesiumMetadataPropertyStatisticsDescription& propertyStatistics :
    ...
    for (const FCesiumMetadataPropertyStatisticValue& statistic :
      ...
      case ECesiumMetadataType::Scalar:
        pParameter =
            NewObject<UMaterialExpressionScalarParameter>(pTargetLayer);
        break;
      case ECesiumMetadataType::Vec2:
      case ECesiumMetadataType::Vec3:
      case ECesiumMetadataType::Vec4:
        pParameter =
            NewObject<UMaterialExpressionVectorParameter>(pTargetLayer);
        break;
      default:
  ...
  if (SetMaterialAttributes->Inputs.Num() <= 1) {
    ...
    SetMaterialAttributes->Inputs[1].Connect(0, pRaymarchNode);
    SetMaterialAttributes->Inputs[1].InputName = "Base Color";

    SetMaterialAttributes->Inputs.EmplaceAt(2, FExpressionInput());
    SetMaterialAttributes->Inputs[2].Connect(3, pBreakFloat4Node);
...
void UCesiumVoxelMetadataComponent::GenerateMaterial() {
  ...
  //       Error,
  //       TEXT(
  //           "Can't regenerate a material layer that has unsaved changes.
  //           Please save your changes and try again."));
  //   return;
  // }
  ...
  // } else {
  //   this->TargetMaterialLayer = CreateMaterialLayer(PackageName,
  //   MaterialName);
  // }
