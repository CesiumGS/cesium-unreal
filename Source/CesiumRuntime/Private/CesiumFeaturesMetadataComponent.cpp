...
static void ClearAutoGeneratedNodes(
  ...
  for (const UMaterialExpressionMaterialFunctionCall* GetFeatureIdNode :
    ...
    if (!Connections.IsEmpty()) {
      MaterialState.ConnectionOutputRemap.Emplace(
          MoveTemp(Key),
  ...
  for (const UMaterialExpressionIf* IfNode : Classification.IfNodes) {
    ...
    if (!Connections.IsEmpty()) {
      MaterialState.ConnectionOutputRemap.Emplace(Key, MoveTemp(Connections));
    }
    ...
    // connections.
    TMap<FString, MaterialGenerationState::ConnectionInput> InputConnections;
    ...
    auto saveIfNodeConnection = [&InputConnections](
      ...
      if (!connectedToAutogeneratedNode) {
        InputConnections.Emplace(
            inputName,
            MaterialGenerationState::ConnectionInput{
                .pUserNode = &input,
                .autogeneratedNodeName = FString(),
                .autogeneratedOutputName = FString()});
      } else if (
        ...
        InputConnections.Emplace(
            ...
            MaterialGenerationState::ConnectionInput{
                .pUserNode = nullptr,
                .autogeneratedNodeName = pCustom->Description,
                .autogeneratedOutputName =
                    pCustom->Outputs[input.OutputIndex].OutputName.ToString()});
      }
    };
    ...
    if (!InputConnections.IsEmpty()) {
      MaterialState.ConnectionInputRemap.Emplace(
          MoveTemp(Key),
...
static void RemapUserConnections(
  ...
  for (UMaterialExpressionMaterialFunctionCall* GetFeatureIdNode :
    ...
    if (!Inputs.IsEmpty()) {
      ...
      TArray<FExpressionInput*>* pConnections =
          MaterialState.ConnectionOutputRemap.Find(Key);
      if (pConnections) {
  ...
  for (UMaterialExpressionIf* IfNode : Classification.IfNodes) {
    ...
    TMap<FString, MaterialGenerationState::ConnectionInput>* pInputConnections =
        MaterialState.ConnectionInputRemap.Find(Key);
    if (pInputConnections) {
      auto connectInput = [pInputConnections, &Classification](
                              const FString& inputName,
                              FExpressionInput& input) {
        const MaterialGenerationState::ConnectionInput* pConnectionInput =
            pInputConnections->Find(inputName);
        if (!pConnectionInput) {
          return;
        }

        if (pConnectionInput->pUserNode) {
          input.Expression = pConnectionInput->pUserNode->Expression;
          input.OutputIndex = pConnectionInput->pUserNode->OutputIndex;
        } else if (!pConnectionInput->autogeneratedNodeName.IsEmpty()) {
          for (UMaterialExpression* pNode : Classification.AutoGeneratedNodes) {
            if (pNode && pNode->GetDescription() ==
                             pConnectionInput->autogeneratedNodeName) {
              TArray<FExpressionOutput>& outputs = pNode->GetOutputs();
              for (int32 i = 0; i < outputs.Num(); i++) {
                if (pConnectionInput->autogeneratedOutputName ==
                    outputs[i].OutputName) {
