...
UMaterialExpressionMaterialFunctionCall* GenerateNodesForFeatureIdTexture(
  ...
  UMaterialExpressionCustom* ConstructVectorValuesFunction =
      NewObject<UMaterialExpressionCustom>(TargetMaterialLayer);
  ConstructVectorValuesFunction->Code = "";
  ConstructVectorValuesFunction->Description =
      "Construct Vector Values For " + SafeName;
  ConstructVectorValuesFunction->MaterialExpressionEditorX = NodeX;
  ConstructVectorValuesFunction->MaterialExpressionEditorY = NodeY;

  int NumInputs = 2;
  int NumOutputs = 1;
  if (Description.bHasKhrTextureTransform) {
    // ScaleOffsetRGB + ScaleOffsetRGBAlpha + Rotation.
    // Rotation is a Vector2, so the extra parameter is not needed.
    NumInputs += 3;
    NumOutputs += 2;
  }

  ConstructVectorValuesFunction->Inputs.Reserve(NumInputs);
  ConstructVectorValuesFunction->Outputs.Reset(NumOutputs);
  ConstructVectorValuesFunction->AdditionalOutputs.Reserve(NumOutputs - 1);
  ConstructVectorValuesFunction->Outputs.Add(
      FExpressionOutput(FName("Channels")));
  ConstructVectorValuesFunction->OutputType =
      ECustomMaterialOutputType::CMOT_Float4;
  ConstructVectorValuesFunction->bShowOutputNameOnPin = true;

  FCustomInput& ChannelsRgbInput = ConstructVectorValuesFunction->Inputs[0];
  ChannelsRgbInput.InputName = FName("Channels_RGB");
  ChannelsRgbInput.Input.Expression = Channels;
  ChannelsRgbInput.Input.OutputIndex = 0;

  FCustomInput& ChannelsAlphaInput =
      ConstructVectorValuesFunction->Inputs.Emplace_GetRef();
  ChannelsAlphaInput.InputName = FName("Channels_A");
  ChannelsAlphaInput.Input.Expression = Channels;
  ChannelsAlphaInput.Input.OutputIndex = 4;

  if (Description.bHasKhrTextureTransform) {
    FCustomInput& ScaleOffsetRgbInput =
        ConstructVectorValuesFunction->Inputs.Emplace_GetRef();
    ScaleOffsetRgbInput.InputName = FName("ScaleOffset_RGB");
    ScaleOffsetRgbInput.Input.Expression = TransformScaleOffset;
    ScaleOffsetRgbInput.Input.OutputIndex = 0;

    FCustomInput& ScaleOffsetAlphaInput =
        ConstructVectorValuesFunction->Inputs.Emplace_GetRef();
    ScaleOffsetAlphaInput.InputName = FName("ScaleOffset_A");
    ScaleOffsetAlphaInput.Input.Expression = TransformScaleOffset;
    ScaleOffsetAlphaInput.Input.OutputIndex = 4;

    FCustomInput& RotationRgbInput =
        ConstructVectorValuesFunction->Inputs.Emplace_GetRef();
    RotationRgbInput.InputName = FName("Rotation_RGB");
    RotationRgbInput.Input.Expression = TransformRotation;
    RotationRgbInput.Input.OutputIndex = 0;

    FCustomOutput& ScaleOffsetOutput =
        ConstructVectorValuesFunction->AdditionalOutputs.Emplace_GetRef();
    ScaleOffsetOutput.OutputName = FName("ScaleOffset");
    ConstructVectorValuesFunction->Outputs.Add(
        FExpressionOutput(ScaleOffsetOutput.OutputName));
    ScaleOffsetOutput.OutputType = ECustomMaterialOutputType::CMOT_Float4;

    FCustomOutput& RotationOutput =
        ConstructVectorValuesFunction->AdditionalOutputs.Emplace_GetRef();
    RotationOutput.OutputName = FName("Rotation");
    ConstructVectorValuesFunction->Outputs.Add(
        FExpressionOutput(RotationOutput.OutputName));
    RotationOutput.OutputType = ECustomMaterialOutputType::CMOT_Float2;

    ConstructVectorValuesFunction->Code +=
        "ScaleOffset = float4(ScaleOffset_RGB.r, ScaleOffset_RGB.g, ScaleOffset_RGB.b, ScaleOffset_A.r);\n";
    ConstructVectorValuesFunction->Code +=
        "Rotation = float2(Rotation_RGB.r, Rotation_RGB.g);\n";
  }

  ConstructVectorValuesFunction->Code +=
      "return float4(Channels_RGB.r, Channels_RGB.g, Channels_RGB.b, Channels_A.r);";

  AutoGeneratedNodes.Add(ConstructVectorValuesFunction);

  NodeY -= 1.75 * Incr;
  NodeX +=
      Incr *
      (GetNameLengthScalar(ConstructVectorValuesFunction->Description) + 1);
  ...
  GetFeatureIdsFromTexture->FunctionInputs[3].Input.Expression =
      ConstructVectorValuesFunction;
  GetFeatureIdsFromTexture->FunctionInputs[3].Input.OutputIndex = 0;

  if (Description.bHasKhrTextureTransform) {
    // Scale + Offset
    GetFeatureIdsFromTexture->FunctionInputs[4].Input.Expression =
        ConstructVectorValuesFunction;
    GetFeatureIdsFromTexture->FunctionInputs[4].Input.OutputIndex = 1;
    // Rotation
    GetFeatureIdsFromTexture->FunctionInputs[5].Input.Expression =
        ConstructVectorValuesFunction;
    GetFeatureIdsFromTexture->FunctionInputs[5].Input.OutputIndex = 2;
  }
