...
UCesiumMetadataPickingBlueprintLibrary::GetMetadataValuesForFace(
  ...
  const FCesiumPrimitiveFeatures& features = pData->Features;
...
bool UCesiumMetadataPickingBlueprintLibrary::FindUVFromHit(
  ...
  if (pData->PositionAccessor.status() !=
      CesiumGltf::AccessorViewStatus::Valid) {
    return false;
  }

  auto accessorIt = pData->TexCoordAccessorMap.find(GltfTexCoordSetIndex);
  if (accessorIt == pData->TexCoordAccessorMap.end()) {
    return false;
  }
  ...
  auto VertexIndices = std::visit(
      CesiumGltf::IndicesForFaceFromAccessor{
          Hit.FaceIndex,
          pData->PositionAccessor.size(),
          pData->pMeshPrimitive->mode},
      pData->IndexAccessor);
  ...
  for (size_t i = 0; i < Positions.size(); i++) {
    auto& Position = pData->PositionAccessor[VertexIndices[i]];
