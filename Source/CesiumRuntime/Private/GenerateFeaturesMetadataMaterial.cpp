...
static const FString GetBlueprintStylePrefix = "Get Blueprint Style For ";
...
FeaturesMetadataClassification::FeaturesMetadataClassification(
  ...
  for (const TObjectPtr<UMaterialExpression>& pNode :
       pMaterialLayer->GetExpressionCollection().Expressions) {
    // Check if this node is marked as autogenerated.
    if (MaterialNodeClassification::IsAutogenerated(*pNode)) {
      this->AutoGeneratedNodes.Add(pNode);

      if (auto* pCustomNode = Cast<UMaterialExpressionCustom>(pNode)) {
        if (pCustomNode->Description.Contains(GetBlueprintStylePrefix)) {
          this->GetBlueprintStyleNodes.Add(pCustomNode);
          continue;
        }
        if (pCustomNode->Description.Contains(GetPropertyValuesPrefix)) {
          this->GetPropertyValueNodes.Add(pCustomNode);
          continue;
        }
        if (pCustomNode->Description.Contains(ApplyValueTransformsPrefix)) {
          this->ApplyValueTransformNodes.Add(pCustomNode);
          continue;
        }
      }

      if (UMaterialExpressionParameter* pParameterNode =
              Cast<UMaterialExpressionParameter>(pNode)) {
        this->ParameterNodes.Add(pParameterNode);
        continue;
      }
      ...
      // value.
      if (UMaterialExpressionIf* pIfNode = Cast<UMaterialExpressionIf>(pNode)) {
        this->IfNodes.Add(pIfNode);
        continue;
      }

      UMaterialExpressionMaterialFunctionCall* pFunctionCallNode =
          Cast<UMaterialExpressionMaterialFunctionCall>(pNode);
      if (!pFunctionCallNode)
        continue;

      const FName& name = pFunctionCallNode->MaterialFunction->GetFName();
      if (name == GetFeatureIdsFromAttributeFunction->GetFName() ||
        ...
        this->GetFeatureIdNodes.Add(pFunctionCallNode);
      }
    } else {
      this->UserAddedNodes.Add(pNode);
    }
  }
}

namespace {
void disconnectGetFeatureIdNode(
    UMaterialExpressionMaterialFunctionCall* pGetFeatureIdNode,
    FExpressionInput* pUserNodeInput,
    TMap<FString, TArray<FExpressionInput*>>& userConnectionMap) {
  CESIUM_ASSERT(
      pGetFeatureIdNode && pGetFeatureIdNode == pUserNodeInput->Expression);
  CESIUM_ASSERT(pUserNodeInput);

  if (!pGetFeatureIdNode->Outputs.Num()) {
    return;
  }

  const auto inputs = pGetFeatureIdNode->FunctionInputs;
  if (!inputs.Num()) {
    // Should not happen, but just in case, this node would be invalid. Break
    // the connection to this node and don't attempt to remap it.
    pUserNodeInput->Expression = nullptr;
    return;
  }
  ...
  // set.
  const auto pParameter =
      Cast<UMaterialExpressionParameter>(inputs[0].Input.Expression);
  FString parameterName;
  if (pParameter) {
    parameterName = pParameter->ParameterName.ToString();
  }

  if (parameterName.IsEmpty()) {
    // In this case, treat the node as invalid. Break
    // the connection to this node and don't attempt to remap it.
    pUserNodeInput->Expression = nullptr;
    return;
  }

  FString key = pGetFeatureIdNode->GetDescription() + parameterName;
  TArray<FExpressionInput*>& connections = userConnectionMap.Contains(key)
                                               ? userConnectionMap[key]
                                               : userConnectionMap.Add(key);
  connections.Add(pUserNodeInput);
  pUserNodeInput->Expression = nullptr;
}

void disconnectCustomNode(
    UMaterialExpressionCustom* pCustom,
    FExpressionInput* pUserNodeInput,
    TMap<FString, TArray<FExpressionInput*>>& userConnectionMap) {
  CESIUM_ASSERT(pCustom && pCustom == pUserNodeInput->Expression);
  CESIUM_ASSERT(pUserNodeInput);

  FExpressionOutput* pOutput = pCustom->GetOutput(pUserNodeInput->OutputIndex);
  if (!pOutput) {
    pUserNodeInput->Expression = nullptr;
    return;
  }

  FString key = pCustom->GetDescription() + pOutput->OutputName.ToString();
  TArray<FExpressionInput*>& connections = userConnectionMap.Contains(key)
                                               ? userConnectionMap[key]
                                               : userConnectionMap.Add(key);
  connections.Add(pUserNodeInput);
...
void disconnectParameterNode(
    UMaterialExpressionParameter* pParameterNode,
    FExpressionInput* pUserNodeInput,
    TMap<FString, TArray<FExpressionInput*>>& userConnectionMap) {
  CESIUM_ASSERT(pParameterNode && pParameterNode == pUserNodeInput->Expression);
  CESIUM_ASSERT(pUserNodeInput);

  FString key = pParameterNode->ParameterName.ToString();
  TArray<FExpressionInput*>& connections = userConnectionMap.Contains(key)
                                               ? userConnectionMap[key]
                                               : userConnectionMap.Add(key);
  connections.Add(pUserNodeInput);
...
void ClearAutoGeneratedNodes(
  ...
  for (UMaterialExpression* pUserNode : Classification.UserAddedNodes) {
    for (FExpressionInputIterator It(pUserNode); It; ++It) {
      const auto pFunctionCall =
          Cast<UMaterialExpressionMaterialFunctionCall>(It.Input->Expression);
      if (Classification.GetFeatureIdNodes.Contains(pFunctionCall)) {
        disconnectGetFeatureIdNode(
            pFunctionCall,
            It.Input,
      ...
      const auto pCustom =
          Cast<UMaterialExpressionCustom>(It.Input->Expression);
      if (Classification.GetBlueprintStyleNodes.Contains(pCustom) ||
          Classification.GetPropertyValueNodes.Contains(pCustom) ||
          Classification.ApplyValueTransformNodes.Contains(pCustom)) {
        disconnectCustomNode(
            pCustom,
            It.Input,
            MaterialState.UserConnectionOutputMap);
        continue;
      ...
      const auto pParameter =
          Cast<UMaterialExpressionParameter>(It.Input->Expression);
      if (Classification.ParameterNodes.Contains(pParameter)) {
        disconnectParameterNode(
            pParameter,
            It.Input,
            MaterialState.UserConnectionOutputMap);
        continue;
  ...
  // statements for null feature IDs. These are handled separately since they
  ...
  for (const UMaterialExpressionIf* IfNode : Classification.IfNodes) {
    ...
    auto saveIfNodeConnection = [&InputConnections](
      ...
      bool connectedToAutogeneratedNode =
          MaterialNodeClassification::IsAutogenerated(*input.Expression);
      ...
      if (!connectedToAutogeneratedNode) {
        InputConnections.Emplace(
          ...
          pCustom && (pCustom->Description.StartsWith(
                      ...
                      pCustom->Description.StartsWith(
                          GetBlueprintStylePrefix,
                          ESearchCase::Type::CaseSensitive))) {
        InputConnections.Emplace(
            inputName,
  ...
  for (UMaterialExpression* AutoGeneratedNode :
    ...
    pMaterialLayer->GetExpressionCollection().RemoveExpression(
        AutoGeneratedNode);
  }
}
...
void RemapUserConnections(
  ...
  // TODO: turn into lambda/helper
  for (UMaterialExpressionCustom* pNode :
       Classification.GetBlueprintStyleNodes) {
    int32 OutputIndex = 0;
    for (const FExpressionOutput& PropertyOutput : pNode->Outputs) {
      FString Key = pNode->Description + PropertyOutput.OutputName.ToString();

      TArray<FExpressionInput*>* pConnections =
          MaterialState.UserConnectionOutputMap.Find(Key);
      if (pConnections) {
        for (FExpressionInput* pConnection : *pConnections) {
          pConnection->Connect(OutputIndex, pNode);
        }
      }
  ...
  for (UMaterialExpressionParameter* pParameterNode :
       Classification.ParameterNodes) {
    FString key = pParameterNode->ParameterName.ToString();
    TArray<FExpressionInput*>* pConnections =
        MaterialState.UserConnectionOutputMap.Find(key);
    if (pConnections) {
      for (FExpressionInput* pConnection : *pConnections) {
        pConnection->Expression = pParameterNode;
      }
    }
  }
}
...
inline FString getFeatureIdCode(const FString& textureName) {
  return "uint _czm_width;\n"
         "uint _czm_height;\n" +
         textureName +
...
struct BlueprintStylingParameter {
  FString Name;
  FString MaterialSuffix;
  ECesiumEncodedMetadataType EncodedType;
};

// Currently there are two styling parameters supported:
// - Color (8-bit RGBA)
// - Show  (boolean)
static const TArray<BlueprintStylingParameter> StylingParameters{
    {"Show", MaterialFeatureShowSuffix, ECesiumEncodedMetadataType::Scalar},
    {"Color", MaterialFeatureColorSuffix, ECesiumEncodedMetadataType::Vec4}};

void GenerateBlueprintStylingNodes(
    const FCesiumFeatureIdSetDescription& FeatureIdSet,
    TArray<UMaterialExpression*>& AutoGeneratedNodes,
    UMaterialFunctionMaterialLayer* pMaterialLayer,
    int32& NodeX,
    int32& NodeY,
    UMaterialExpression* pGetFeatureExpression) {
  int32 BeginSectionX = NodeX;
  // This value is used by parameters on the left side of the
  // "GetPropertyValues" function.
  int32 PropertyDataSectionY = NodeY - 0.5 * Incr;

  auto* pCustomFunction = NewObject<UMaterialExpressionCustom>(pMaterialLayer);
  pCustomFunction->Inputs.Reserve(StylingParameters.Num() + 1);
  pCustomFunction->Outputs.Reset(3);
  pCustomFunction->Outputs.Add(FExpressionOutput(TEXT("Feature ID")));
  pCustomFunction->bShowOutputNameOnPin = true;
  pCustomFunction->Code = "";
  pCustomFunction->Description = GetBlueprintStylePrefix + FeatureIdSet.Name;
  pCustomFunction->MaterialExpressionEditorX = NodeX;
  pCustomFunction->MaterialExpressionEditorY = NodeY;
  AutoGeneratedNodes.Add(pCustomFunction);

  int32 functionWidth =
      Incr * GetNameLengthScalar(pCustomFunction->Description);
  ...
  FCustomInput& FeatureIDInput = pCustomFunction->Inputs[0];
  FeatureIDInput.InputName = FName("FeatureID");
  FeatureIDInput.Input.Expression = pGetFeatureExpression;

  pCustomFunction->AdditionalOutputs.Reserve(2);

  FString featureIdSetName = createHlslSafeName(FeatureIdSet.Name);

  bool isFirstParameter = true;
  for (const BlueprintStylingParameter& Parameter : StylingParameters) {
    PropertyDataSectionY += Incr;

    FString dataTextureName = Parameter.Name + MaterialPropertyDataSuffix;
    if (isFirstParameter) {
      pCustomFunction->Code = getFeatureIdCode(dataTextureName);
      isFirstParameter = false;
    }

    auto* pColorData =
        NewObject<UMaterialExpressionTextureObjectParameter>(pMaterialLayer);
    pColorData->ParameterName =
        FName(featureIdSetName + Parameter.MaterialSuffix);
    pColorData->MaterialExpressionEditorX = BeginSectionX;
    pColorData->MaterialExpressionEditorY = PropertyDataSectionY;
    AutoGeneratedNodes.Add(pColorData);

    MaximumPropertyDataSectionX = FMath::Max(
        MaximumPropertyDataSectionX,
        Incr * GetNameLengthScalar(pColorData->ParameterName));

    FCustomInput& PropertyInput = pCustomFunction->Inputs.Emplace_GetRef();
    PropertyInput.InputName = FName(dataTextureName);
    PropertyInput.Input.Expression = pColorData;

    FCustomOutput& PropertyOutput =
        pCustomFunction->AdditionalOutputs.Emplace_GetRef();

    PropertyOutput.OutputName = FName(Parameter.Name);
    pCustomFunction->Outputs.Add(FExpressionOutput(PropertyOutput.OutputName));

    FString swizzle = GetSwizzleForEncodedType(Parameter.EncodedType);
    PropertyOutput.OutputType =
        GetOutputTypeForEncodedType(Parameter.EncodedType);

    // Example:
    // "color = color_DATA.Load(int3(_czm_pixelX, _czm_pixelY, 0)).rgba;"
    pCustomFunction->Code += Parameter.Name + " = " + dataTextureName +
                             ".Load(int3(_czm_pixelX, _czm_pixelY, 0))" +
                             swizzle + ";\n";
  }

  // Shift the X of GetPropertyValues depending on the width of the data
  // parameters.
  pCustomFunction->MaterialExpressionEditorX +=
      MaximumPropertyDataSectionX + Incr;
  NodeX = pCustomFunction->MaterialExpressionEditorX + functionWidth;

  // Return the feature ID.
  pCustomFunction->OutputType = ECustomMaterialOutputType::CMOT_Float1;
  pCustomFunction->Code += "return FeatureID;";

  NodeX = pCustomFunction->MaterialExpressionEditorX + functionWidth + Incr;
  NodeY = PropertyDataSectionY + Incr;
}
...
void GenerateNodesForPropertyTable(
  ...
  for (const FCesiumPropertyTablePropertyDescription& Property :
    ...
    if (!foundFirstProperty) {
      ...
      // count.
      GetPropertyValuesFunction->Code = getFeatureIdCode(PropertyDataName);
...
void GenerateStatisticsNodes(
    const FCesiumMetadataClassStatisticsDescription& ClassStatistics,
    TArray<UMaterialExpression*>& AutoGeneratedNodes,
    UMaterialFunctionMaterialLayer* TargetMaterialLayer,
    int32& NodeX,
    int32& NodeY) {
  for (const FCesiumMetadataPropertyStatisticsDescription& propertyStatistics :
    ...
    for (const FCesiumMetadataPropertyStatisticValue& statisticValue :
      ...
      FCesiumMetadataValueType valueType =
          UCesiumMetadataValueBlueprintLibrary::GetValueType(
      ...
      pParameter->ParameterName = FName(getNameForStatistic(
          ClassStatistics.Id,
          propertyStatistics.Id,
      ...
      AutoGeneratedNodes.Add(pParameter);
      NodeY += 0.75 * Incr;
    }
  }
}
...
void GenerateMaterialNodes(
  ...
  const TSet<FString> PropertyTextureNames =
      Description.PrimitiveMetadata.PropertyTextureNames;
  ...
  // Generate nodes for statistics first so they're easier to find.
  for (const FCesiumMetadataClassStatisticsDescription& classStatistics :
       Description.Statistics) {
    GenerateStatisticsNodes(
        classStatistics,
  ...
  for (const FCesiumFeatureIdSetDescription& featureIdSet : FeatureIdSets) {
    ...
    UMaterialExpression* pLastNode = nullptr;
    if (featureIdSet.Type == ECesiumFeatureIdSetType::Texture) {
      pLastNode = GenerateNodesForFeatureIdTexture(
          featureIdSet,
          MaterialState.AutoGeneratedNodes,
          pMaterialLayer,
          FunctionLibrary,
      ...
      pLastNode = GenerateInstanceNodes(
          MaterialState.AutoGeneratedNodes,
          pMaterialLayer,
          FunctionLibrary.GetFeatureIdsFromInstance,
          NodeX,
      ...
      pLastNode = GenerateNodesForFeatureIdAttribute(
          featureIdSet,
          MaterialState.AutoGeneratedNodes,
          pMaterialLayer,
          FunctionLibrary.GetFeatureIdsFromAttribute,
    ...
    if (featureIdSet.StylingMode == ECesiumFeatureStylingMode::Blueprint) {
      GenerateBlueprintStylingNodes(
          featureIdSet,
          MaterialState.AutoGeneratedNodes,
          pMaterialLayer,
          NodeX,
          NodeY,
          pLastNode);
    } else if (!featureIdSet.PropertyTableName.IsEmpty()) {
  ...
  if (!InputMaterial) {
    InputMaterial = NewObject<UMaterialExpressionFunctionInput>(pMaterialLayer);
  ...
  if (!SetMaterialAttributes) {
    SetMaterialAttributes =
        NewObject<UMaterialExpressionSetMaterialAttributes>(pMaterialLayer);
  ...
  if (!OutputMaterial) {
    OutputMaterial =
        NewObject<UMaterialExpressionFunctionOutput>(pMaterialLayer);
